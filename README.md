...
* 0318 : [01] servlet원리, servlet을 이용한 폼 데이터의 처리, Servlet 실습, @WebServlet annotation을 이용한 Servlet 3.0 이상에서의 서블릿의 선언  
  ...
  * 1) JSP는 실행 시 Servlet class로 변경되어 실행, 출력목적이 아닌 대부분 처리 목적(정확한 처리)  
  * 2) Servlet은 자동화 문제로 불편함 Spring으로 자동화 문제 해결  
  * 3) Servlet code는 doGet, doPost method를 명시하여 src에 mapping  
